...
def word_write_command_to_docx(doc, cmd, lines, font_size_cmd, font_size_lines):
    first_line = True
    paragraph = doc.add_paragraph()
    header = paragraph.add_run()
    header.add_text(cmd)
...
def word_log_txt_file_to_docx(log_file_full, title, device):
    if device == "ASR":
        word_log_txt_file_to_docx_asr(log_file_full, title)
    elif device in ("cBR8", "uBR10K"):
        word_log_txt_file_to_docx_cmts(log_file_full, title)


def word_log_txt_file_to_docx_asr(log_file_full, title):
    doc = Document()
    word_docx_add_highlighted_paragraph_line(doc, title, 'Times New Roman', 12, True)
