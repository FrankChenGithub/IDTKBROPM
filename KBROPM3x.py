...
# TODO 20200915 FrankChen Migration log
#  20200915-01: import urllib2 >> import urllib.request as urllib2
#  20200915-02: print "..." >> print("...") function call
#  20200915-03: remove reload(sys) and sys.setdefaultencoding('utf-8')
#  20200915-04: change path (unresolved reference) to FILENAME
#  20200915-05: change frame (unresolved reference) to sheet_name_ip and kbro_pm_xlsx_file_name
#  20200915-06: telnet 2x string > 3x bytes
#               read_until 字串加 .encode()
#               write(xxx + '\n') > write(xxx.encode('ascii') + b'\n')
#               write("a string" + '\n') > write(b"a string \n")
#               data = telnet.read_all().split()
#			>>  data = telnet.read_all().decode('ascii').split()
#  20200915-07: open file for output with 'wb' >> "w"
#  20200915-08 新的 urllib.request 的 urlopen 如果需要取值，也需要 decode('ascii') bytes to string

import re, os, sys, time, socket, datetime, xlrd, telnetlib, linecache
import shutil
...
# TODO 20200915-03 FRANK Comment utf-8 related
# reload(sys)
...
def get_content(FILENAME, HOST):
    ...
    if os.path.exists(FILENAME):
        ...
        bk = xlrd.open_workbook(kbro_pm_xlsx_file_name)
        ...
        for file_data_line in range(len(file_data)):
            for execl_line in range(1, nrows):
...
def telNetCall(IP, HOST, SO, DEVICE, ID="", PW=""):
    global telnet
    output = " "
    data = " "
    # 如果沒有值傳進來，則使用預設帳密
    ...
    if (DEVICE == "uBR10K") or (DEVICE == "cBR8") or (DEVICE == "ASR") or (DEVICE == "DTI") or (DEVICE == "Switch"):
        ...
        shxrange = range(bk.nsheets)
        sh = bk.sheet_by_name(DEVICE)

        nrows = sh.nrows
        ncols = sh.ncols
        print("\n#################################################")
        print("command nrows = %d, ncols = %d" % (nrows, ncols))
    print("ASR 01 @", datetime.datetime.now())
    ###########telnet ASR##########
    if (DEVICE == "ASR"):
    ...
    print("ASR 02 @", datetime.datetime.now())
    if (DEVICE == "uBR10K") or (DEVICE == "cBR8") or (DEVICE == "ASR") or (DEVICE == "DTI") or (DEVICE == "Switch"):
    ...
    # login CMTS or ASR
    ############################################################
    print("ASR 03 issuing command @", datetime.datetime.now())
    if (DEVICE == "uBR10K") or (DEVICE == "cBR8") or (DEVICE == "ASR") or (DEVICE == "Switch"):
        ...
        for i in range(1, nrows):
            row_command = sh.cell_value(i, 0)
            print("command is %s" % (row_command))
            telnet.write(row_command.encode('ascii') + b'\r\n')

        telnet.write(b"exit \r\n")
        print("exit")
        print("ASR 04 finished issuing command @", datetime.datetime.now())
        output = " "
        # todo not to decode
        output = telnet.read_all().decode('ascii')
        print("ASR 05 get and decode data @", datetime.datetime.now())
        fp = open(FILENAME, "w")
        fp.write(output)
        fp.close
        print("ASR 05 finish files @", datetime.datetime.now())
        if (DEVICE == "ASR"):
            content = get_content(FILENAME, HOST)
            convert_pdf(FOLDER, FILENAMEPDF)
        print("ASR 06 pdf file converted @", datetime.datetime.now())
    ############################################################
    # login DTI
    ############################################################
...
def get_pnr_http_screenshot(IP, HOST, SO, DEVICE, id, pw):
    ...
    png_folder = "LOG/%s/%s/%s" % (SO, DEVICE, HOST)
...
sheet_name_ip = "IP"
try:
    sh = bk.sheet_by_name(sheet_name_ip)
except:
    # print("no sheet in %s named Sheet" % frame)
    print("No sheet in [{0}] named {1}".format(kbro_pm_xlsx_file_name, sheet_name_ip))

nrows = sh.nrows
ncols = sh.ncols
print("IP nrows = %d, ncols = %d" % (nrows, ncols))

for i in range(1, nrows):
    row_IP = sh.cell_value(i, 0)
    row_HOST = sh.cell_value(i, 1)
    row_SO = sh.cell_value(i, 2)
    row_DEVICE = sh.cell_value(i, 3)
    if 6 <= sh.ncols:
        row_id = sh.cell_value(i, 4)
        row_pw = sh.cell_value(i, 5)
        if len(row_id) > 0 or len(row_pw) > 0:
            print("ASSIGNED from xlsx", row_id, row_pw)
    else:
        row_id = ""
        row_pw = ""
    try:
        # policymap = telNetCall(row_IP, row_HOST, row_SO, row_DEVICE)
        if row_DEVICE.upper() == "PNR":
            get_pnr_http_screenshot(row_IP, row_HOST, row_SO, row_DEVICE, row_id, row_pw)
        else:
            start_time = datetime.datetime.now()
            print(i, row_IP, row_HOST, row_SO, row_DEVICE, row_id, row_pw)
            print("row", i, "start@", start_time)
            telNetCall(row_IP, row_HOST, row_SO, row_DEVICE, row_id, row_pw)
            print("total time of row", i, (datetime.datetime.now() - start_time).total_seconds())
    except socket.timeout:
        f = open(fail_log_file, "w+")
        print("\n###############################")
        print("HOST %s is timeout" % row_HOST)
        print("###############################\n")
        f.write("socket.timeout," + row_IP + "," + row_HOST + "," + row_SO + "," + row_DEVICE + "," + row_id + "," + row_pw + "\n")
        f.close()
    except OSError:
        f = open(fail_log_file, "w+")
        print("\n###############################")
        print("HOST %s is OSError" % row_HOST)
        print("###############################\n")
        f.write("OSError," + row_IP + "," + row_HOST + "," + row_SO + "," + row_DEVICE + "," + row_id + "," + row_pw + "\n")
        f.close()
    except:
        f = open(fail_log_file, "w+")
        print("\n###############################")
        print("HOST %s is OSError" % row_HOST)
        print("###############################\n")
        f.write("UnknownError," + row_IP + "," + row_HOST + "," + row_SO + "," + row_DEVICE + "," + row_id + "," + row_pw + "\n")
        f.close()
